...
| Deux droits distincts | **Gérer un bien** (arbitrer séjours et dépenses) et **administrer la plateforme** (version, mises à jour, réglages d'instance, courriel, journal, calendrier scolaire) ne se recouvrent pas. Le second est un attribut de la personne, pas un rôle : il n'ouvre **aucun bien**, et un administrateur non rattaché ne voit toujours rien. L'export complet, qui emporte toute la base, reste au gérant. Le droit ne se donne qu'entre administrateurs, on ne retire jamais le dernier, et la migration 010 l'a donné aux gérants en poste pour qu'aucune instance ne se retrouve close. | `acces/roles.ts`, `systeme/administrateurs.ts` |
| Réglages d'instance | Modifiables par un administrateur de la plateforme, **section Sécurité comprise**. C'est un choix assumé : `membreFoyerVoitDepenses` et la durée d'affichage des codes y vivent, donc un administrateur peut s'ouvrir une vue qu'il n'avait pas. Contrepartie retenue plutôt que l'interdiction : **chaque changement est écrit au journal d'audit** avec son auteur, l'ancienne et la nouvelle valeur. | `parametres/routes.ts` |
...
| Mise à jour depuis l'interface | **Le service n'exécute jamais rien.** Il écrit `.maj-declencheur` dans son répertoire de données ; une unité systemd `path` **appartenant à root** lance le script qui télécharge, compile et redémarre. Le service garde son durcissement (`ProtectSystem=strict`, aucun droit d'écrire dans son code, aucun `sudo`). Le bouton n'existe que si l'assistant root est installé (`MAJ_AUTO=true`), est réservé aux gérants, et **redemande le mot de passe** : ce bouton fait exécuter du code en root, un jeton dérobé sur un téléphone déverrouillé ne doit pas suffire. Un état « en cours » qui cesse de progresser redevient un échec au bout de quinze minutes, sinon l'interface reste bloquée pour toujours. | `systeme/maj.ts`, `systeme/versions.ts`, `deploy/lxc/maj.sh` |
| Réseau sortant | **Trois adresses, et rien d'autre** : le relais SMTP configuré, `api.github.com` pour la version publiée, `data.education.gouv.fr` pour le calendrier scolaire. Chacune est **en dur dans le code**, hôte revérifié après redirection, temporisation courte, taille plafonnée, transport injecté donc testable sans réseau. Le dépôt interrogé se change par `MDF_DEPOT_GITHUB`, jamais depuis l'interface, sinon la mise à jour deviendrait un moyen d'installer n'importe quel code. **Aucune donnée de la famille ne sort** : ces appels demandent, ils ne racontent rien, et il n'y a ni télémétrie ni compteur d'usage. Polices et icônes restent dans le dépôt : la page ne charge rien de l'extérieur. La seule écriture automatique est le calendrier scolaire, et elle **n'ajoute que les années absentes** : une année validée par un gérant n'est jamais réécrite. Aucune installation ne se fait sans mot de passe. | `calendrier/telechargement.ts`, `calendrier/rafraichissement.ts`, `systeme/depot.ts`, `systeme/veille.ts` |
