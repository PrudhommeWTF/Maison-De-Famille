...
| Réseau sortant | **Aucun**, sauf le relais SMTP configuré. Polices et icônes sont dans le dépôt. Deux exceptions **éteintes par défaut**. Le réglage `majVerification` autorise un appel vers `api.github.com` pour savoir s'il existe une version plus récente ; le dépôt interrogé se change par `MDF_DEPOT_GITHUB`, jamais depuis l'interface, sinon la mise à jour deviendrait un moyen d'installer n'importe quel code. Le réglage `vacancesTelechargement` autorise un appel vers `data.education.gouv.fr` pour récupérer le calendrier scolaire. Adresse en dur, hôte revérifié après redirection, temporisation de 20 s, taille plafonnée à 8 Mo, réservé aux gérants, et **aucune écriture sans confirmation humaine**. Allumer ce réglage oblige à rouvrir la sortie réseau du conteneur, et fait connaître l'existence de l'instance au portail. | `calendrier/telechargement.ts` |
